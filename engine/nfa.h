...
public:
  ...
  bool match(const std::string& str) {
    // maybe modify this to encode if the state is visited from an epsilon transitions.
    std::stack<std::pair<std::size_t, std::shared_ptr<State>>> stack;
    std::map<State*, std::size_t> visited;
    stack.push({ 0, start_state });

    while (!stack.empty()) {
      std::pair<std::size_t, std::shared_ptr<State>> current = stack.top();
      visited.insert({ current.second.get(), current.first });
      stack.pop();

      if (current.second->accepting == true && current.first == str.size()) {
        return true;
      }

      for (std::pair<char, std::vector<std::shared_ptr<State>>> trn_pair : current.second->transitions) {
        if (current.first < str.size() && str.at(current.first) == trn_pair.first) {
          for (std::shared_ptr<State> s : trn_pair.second) {
            stack.push(std::make_pair(current.first + 1, s));
          }
        }
      }
      for (std::shared_ptr<State> s : current.second->epsilon_transitions) {
        std::map <State*, std::size_t>::iterator v = visited.find(s.get());
        if (v != visited.end() && v->second == current.first) {
          return false;
        }
        stack.push(std::make_pair(current.first, s));
      }
